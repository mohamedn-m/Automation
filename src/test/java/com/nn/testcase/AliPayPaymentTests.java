...
public class AliPayPaymentTests extends BaseTest {
    ...
    public void adminLogin(@Optional("allipay") String userName, @Optional("wordpress") String password) {
        wooCommerce.getAdminPage().openAdminPage();
        wooCommerce.getAdminPage().adminLogin(userName, password);
    }

    @Test(priority = 1, description = "Check whether the Alipay  payment order placed successully and chargeback, credit and refund events executed successfully",retryAnalyzer = RetryListener.class)
    ...
    @Test(priority = 2, description = "Check whether Alipay payment order placed successfully by communication break and executing payment event",retryAnalyzer = RetryListener.class)
    public void successOrder(){
        setPaymentConfiguration(true,true);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().setBillingAddressCheckout(ExcelHelpers.addressChina());
        wooCommerce.getCheckoutPage().isAlipayDisplayed();
        wooCommerce.getCheckoutPage().verifyalipayTestModeDisplayed(true);
    ...
    @Test(priority = 3, description = "Check whether Alipay payment order failed successfully by communication break and executing payment event",retryAnalyzer = RetryListener.class)
    ...
    public void guestOrder(){
        ...
        wooCommerce.getAdminPage().adminLogin("allipay", "wordpress");
    ...
    @Test(priority = 5,description = "Check whether Alipay payment logo displayed ")
    public void verifyAlipayLogoDisplayed(){
        wooCommerce.getAdminPage().ifLoggedOutLogin("allipay","wordpress");
        setPaymentConfiguration(true,false);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isAlipayDisplayed();
        wooCommerce.getCheckoutPage().isLogoDisplayed("Alipay");
    }
    ...
    public void deactivatePaymentVerifyCheckout(){
        ...
        wooCommerce.getSettingsPage().activatePayment(ALIPAY,false);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isPaymentDisplayed(ALIPAY,false);
    }
    ...
    @Step("Communication break at AliPay redirect page and Get Pending order number")
    public  String communicationBreakGetPendingOrderNumber(){
        wooCommerce.getCheckoutPage().clickPlaceOrderBtn();
        wooCommerce.getCheckoutPage().waitForAliPayRedirectionPage();
        wooCommerce.getMyAccountPage().loadOrders();
