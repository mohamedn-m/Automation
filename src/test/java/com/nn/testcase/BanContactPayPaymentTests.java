...
public class BanContactPayPaymentTests extends BaseTest {
    ...
    public void adminLogin(@Optional("bancontact") String userName, @Optional("wordpress") String password) {
        wooCommerce.getAdminPage().openAdminPage();
        wooCommerce.getAdminPage().adminLogin(userName, password);
    }

    @Test(priority = 1, description = "Check whether the Bancontact  payment order placed successully and chargeback, credit and refund events executed successfully",retryAnalyzer = RetryListener.class)
    ...
    @Test(priority = 2, description = "Check whether Bancontact payment order placed successfully by communication break and executing payment event",retryAnalyzer = RetryListener.class)
    public void successOrder(){
        setPaymentConfiguration(true,true);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isBancontactDisplayed();
        wooCommerce.getCheckoutPage().verifyBancontactTestModeDisplayed(true);
    ...
    @Test(priority = 3, description = "Check whether Bancontact payment order failed successfully by communication break and executing payment event",retryAnalyzer = RetryListener.class)
    ...
    public void guestOrder(){
        ...
        wooCommerce.getAdminPage().adminLogin("bancontact", "wordpress");
    ...
    @Test(priority = 5,description = "Check whether Bancontact payment logo displayed ")
    public void verifyBancontactLogoDisplayed(){
        wooCommerce.getAdminPage().ifLoggedOutLogin("bancontact","wordpress");
        setPaymentConfiguration(true,false);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isBancontactDisplayed();
        wooCommerce.getCheckoutPage().isLogoDisplayed("Bancontact");
    }
    ...
    public void transactionAbort() {
        ...
        cancelBanContactPaymentRedirectionPage();
    ...
    public void deactivatePaymentVerifyCheckout(){
        ...
        wooCommerce.getSettingsPage().activatePayment(BANCONTACT,false);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isPaymentDisplayed(BANCONTACT,false);
    }
    ...
    @Step("Communication break at Bancontact redirect page and Get Pending order number")
    public  String communicationBreakGetPendingOrderNumber(){
        wooCommerce.getCheckoutPage().clickPlaceOrderBtn();
        wooCommerce.getCheckoutPage().waitForBancontactPayRedirectionPage();
        wooCommerce.getMyAccountPage().loadOrders();
    ...
    public void cancelBanContactPaymentRedirectionPage() {
