...
public class GiroPayPaymentTests extends BaseTest {
    ...
    public void adminLogin(@Optional("giropay") String userName, @Optional("wordpress") String password) {
        wooCommerce.getAdminPage().openAdminPage();
        wooCommerce.getAdminPage().adminLogin(userName, password);
    }

    @Test(priority = 1, description = "Check whether the Giropay payment order placed successully and chargeback, credit and refund events executed successfully",retryAnalyzer = RetryListener.class)
    ...
    @Test(priority = 2, description = "Check whether Giropay payment order placed successfully by communication break and executing payment event",retryAnalyzer = RetryListener.class)
    public void successOrder(){
        setPaymentConfiguration(true,true);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isGiropayDisplayed();
        wooCommerce.getCheckoutPage().verifyGiropayTestModeDisplayed(true);
    ...
    @Test(priority = 3, description = "Check whether Giropay payment order failed successfully by communication break and executing payment event",retryAnalyzer = RetryListener.class)
    ...
    public void guestOrder(){
        ...
        wooCommerce.getAdminPage().adminLogin("giropay", "wordpress");
    ...
    @Test(priority = 5,description = "Check whether Giropay payment logo displayed ")
    public void verifyGiropayLogoDisplayed(){
       // verifyGlobalConfiguration();
        wooCommerce.getAdminPage().ifLoggedOutLogin("giropay","wordpress");
        setPaymentConfiguration(true,true);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isGiropayDisplayed();
        wooCommerce.getCheckoutPage().isLogoDisplayed("giropay");
    }
    ...
    public void deactivatePaymentVerifyCheckout(){
        ...
        wooCommerce.getSettingsPage().activatePayment(GIROPAY,false);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isPaymentDisplayed(GIROPAY,false);
    }
    ...
    public  String communicationBreakGetPendingOrderNumber(){
        wooCommerce.getCheckoutPage().clickPlaceOrderBtn();
        wooCommerce.getCheckoutPage().waitForGiroPayRedirectionPage();
        wooCommerce.getMyAccountPage().loadOrders();
