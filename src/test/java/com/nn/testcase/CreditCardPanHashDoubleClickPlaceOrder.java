...
public class CreditCardPanHashDoubleClickPlaceOrder extends BaseTest {
    ...
    public void adminLogin(@Optional("cc1") String userName, @Optional("wordpress") String password) {
    ...
    public void doubleClickOrderBtn(){
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isCreditCardDisplayed();
        wooCommerce.getCheckoutPage().fillCreditCardForm(wooCommerce.getTestData().get("CardNumberDirect"),
                wooCommerce.getTestData().get("ExpDate"),
                wooCommerce.getTestData().get("CVC"));
        wooCommerce.getCheckoutPage().doubleclickPlaceOrderBtn();
    }
