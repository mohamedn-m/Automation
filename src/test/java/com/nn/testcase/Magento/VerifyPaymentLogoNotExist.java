...
import static com.nn.Magento.Constants.PRODUCT_PRZELEWY24_PAY;
...
public class VerifyPaymentLogoNotExist extends BaseTest {
    ...
    public void verifyPaymentLogoBlik(){
        ...
        magentoPage.getMyAccountPage().changeCurrency("PLN");
        magentoPage.getCheckoutPage().load().openCheckoutPage();
        magentoPage.getCheckoutPage().isLogoDisplayed("Blik",false);
    }
    ...
    public void verifyPaymentLogoPayconiq(){
        ...
        navigateCheckout(MagentoAPIs.getCustomerEmail());
        magentoPage.getCheckoutPage().isLogoDisplayed("Payconiq",false);
    }
    @Test(priority = 26,description = "Check whether the Payconiq payment logo displayed as per admin portal configurations ", retryAnalyzer = RetryListener.class)
    public void verifyPaymentLogoMBWay(){
        magentoPage.getShopUserLoginPage().logout();
        createCustomer(MBWAY);
        addProductToCart(PRODUCT_MBWAY,1);
        navigateCheckout(MagentoAPIs.getCustomerEmail());
        magentoPage.getCheckoutPage().isLogoDisplayed("MB Way",false);
    }
}
