...
public class MBWay extends BaseTest {
    Shopware shopwareMBWay = Shopware.builder()
    ...
    @Test(priority = 1, description = "Check whether the MBWay payment order placed successfully and partial refund event executed successfully")
    public void firstOrder() {
        ...
        verifyEquals(paymentName, getPaymentName(MBWAY), PAYMENT_NAME_IN_SUCCESS_PAGE);
        //My account page
        ...
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(MBWAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(MBWAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
        ...
        var partialRefund = shopwareMBWay.getCallback().transactionRefund(tid, String.valueOf(Integer.parseInt(totalAmount) / 2));
    ...
    @Test(priority = 2, description = "Check whether the MBWay payment order placed successfully and full refund event executed successfully")
    public void secondOrder() {
        ...
        var partialRefund = shopwareMBWay.getCallback().transactionRefund(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
    ...
    @Test(priority = 3, description = "Check whether the MBWay payment order placed successfully through communication break")
    public void thirdOrder(){
        ...
        shopware.getIframeV13().isMBWAYDetailsFilled(shopware.getTestData().get("MobileNO"));
        shopware.getCheckoutPage().waitForMBWAYRedirectionPage();
        shopware.getNovalnetAdminPortal().openNovalnetAdminPortal();
        ...
        shopwareMBWay.getCallback().communicationBreakSuccess(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
        verifyEquals(shopware.getMyAccountPage().getOrderStatus(orderNumber), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_MY_ACCOUNT_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(MBWAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE); // failed due to payment name MB Way
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(MBWAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
    ...
    @Test(priority = 4, description = "Check whether the MBWAY payment order placed & failed by communication break")
    public void fourthOrder(){
        ...
        String tid = shopware.getNovalnetAdminPortal().getTID(ShopwareAPIs.getInstance().getCustomerEmail());
        String orderNumber = TID_Helper.getOrderNumber(tid);
        shopwareMBWay.getCallback().communicationBreakFailure(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
    ...
    @Test(priority = 5, description = "Verify the displayed error text on the checkout page in case of transaction aborted, leading to a change in the order status to failed")
    public void fifthOrder(){
        ...
        shopware.getCheckoutPage().clickSubmitOrderBtn();
        shopware.getIframeV13().isMBWAYDetailsFilled(shopware.getTestData().get("MobileNO"));
        shopware.getCheckoutPage().cancelOrderWithMBWay();
    ...
    public void guestOrder(){
        ...
        shopware.getCustomerLoginPage().guestRegister("Portugal");
        shopware.getCheckoutPage()
                .load()
                .enterIframe()
                .isPaymentDisplayed(MBWAY)
        ...
        shopware.getCheckoutPage().clickSubmitOrderBtn();
        shopware.getIframeV13().isMBWAYDetailsFilled(shopware.getTestData().get("MobileNO"));
        shopware.getCheckoutPage().placeOrderWithMBWay();
        shopware.getTxnInfo().putAll(shopware.getCheckoutPage().getSuccessPageTransactionDetails());
        var initialComment = shopware.getTxnInfo().get("Comments");
        var tid = shopware.getTxnInfo().get("TID");
        var orderNumber = shopware.getTxnInfo().get("OrderNumber");
    ...
    @Test(priority = 6, description = "Verify whether the error alert displayed for the empty value in mobile number field")
    public void mbWayValidation1(){
        shopware.getCustomerLoginPage().logout();
        ShopwareAPIs.getInstance().createCustomer(MBWAY);
        shopware.getCustomerLoginPage().load().login(ShopwareAPIs.getInstance().getCustomerEmail());
        ShopwareAPIs.getInstance().addProductToCart(SW_PRODUCT_02);
        ...
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn();
        verifyEquals(shopware.getCheckoutPage().getInvalidMobileNumberError(), getInvalidMobileNumberError(), "Verify invalid mobile number error message in checkout page");
    }
}
