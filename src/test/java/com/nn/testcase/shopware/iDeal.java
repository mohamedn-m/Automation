...
public class iDeal extends BaseTest {
    Shopware shopwareIdeal = Shopware.builder()
    ...
    @Test(priority = 1, description = "Check whether the iDeal payment order placed successfully and refund, chargeback, and credit events executed successfully")
    public void firstOrder() {
        ...
        verifyEquals(paymentName, getPaymentName(IDEAL), PAYMENT_NAME_IN_SUCCESS_PAGE);
        //My account page
        ...
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(IDEAL), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(IDEAL), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
        ...
        var partialRefund = shopwareIdeal.getCallback().transactionRefund(tid, String.valueOf(Integer.parseInt(totalAmount) / 2));
        ...
        //chargeback
        var chargebackComment = shopwareIdeal.getCallback().chargeback(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        ...
        var onlineTransferCredit = shopwareIdeal.getCallback().onlineTransferCredit(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.CHARGEBACK.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
        ...
        var debtCollectionCreditCard = shopwareIdeal.getCallback().debtCollectionDE(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.CHARGEBACK.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
        ...
        var bankTransferByEndCustomer = shopwareIdeal.getCallback().bankTransferByEndCustomer(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.CHARGEBACK.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
    ...
    @Test(priority = 2, description = "Check whether the iDeal payment order placed successfully and full refund events executed successfully")//,retryAnalyzer = RetryListener.class)
    public void secondOrder(){
        ...
        var refund = shopwareIdeal.getCallback().transactionRefund(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
    ...
    public void thirdOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().waitForIDealRedirectionPage();
        shopware.getNovalnetAdminPortal().openNovalnetAdminPortal();
        ...
        shopwareIdeal.getCallback().communicationBreakSuccess(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
        verifyEquals(shopware.getMyAccountPage().getOrderStatus(orderNumber), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_MY_ACCOUNT_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(IDEAL), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
    ...
    public void fourthOrder(){
        ...
        String tid = shopware.getNovalnetAdminPortal().getTID(ShopwareAPIs.getInstance().getCustomerEmail());
        String orderNumber = TID_Helper.getOrderNumber(tid);
        shopwareIdeal.getCallback().communicationBreakFailure(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
    ...
    public void fifthOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().cancelOrderWithIDeal();
    ...
    public void guestOrder(){
        ...
        shopware.getCustomerLoginPage().guestRegister("Netherlands");
        shopware.getCheckoutPage()
                .load()
                .enterIframe()
                .isPaymentDisplayed(IDEAL)
