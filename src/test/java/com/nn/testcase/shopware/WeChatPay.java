...
import static com.nn.callback.CallbackProperties.WECHATPAY;
...
public class WeChatPay extends BaseTest {
    ...
    Shopware shopwareWeChatPay = Shopware.builder()
    ...
    @Test(priority = 1, description = "Check whether the WECHATPAY payment order placed successfully and full refund event executed successfully")
    public void firstOrder() {
        ...
        verifyEquals(paymentName, getPaymentName(WECHATPAY), PAYMENT_NAME_IN_SUCCESS_PAGE);
        //My account page
        ...
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(WECHATPAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(WECHATPAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
        ...
        var refund = shopwareWeChatPay.getCallback().transactionRefund(tid, totalAmount);
    ...
    @Test(priority = 2, description = "Check whether the WECHATPAY payment order placed successully and Callback partial refund events executed successfully")
    public void secondOrder(){
        ...
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
        //Refund partial via callback
        var refund = shopwareWeChatPay.getCallback().transactionRefund(tid, String.valueOf(Integer.parseInt(totalAmount)/2));
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
        verifyContains(shopware.getOrdersPage().getComments(), getCallbackResponse(refund), TRANSACTION_COMMENT_IN_ADMIN_ORDER_DETAIL_PAGE);
        shopware.getMyAccountPage().openOrdersPage().expandOrder(orderNumber);
    ...
    @Test(priority = 3, description = "Check whether the WECHATPAY payment order placed successfully by communication break",retryAnalyzer = RetryListener.class)
    public void thirdOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().waitForWeChatPayRedirection();
        shopware.getNovalnetAdminPortal().openNovalnetAdminPortal();
        ...
        shopwareWeChatPay.getCallback().communicationBreakSuccess(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
        verifyEquals(shopware.getMyAccountPage().getOrderStatus(orderNumber), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_MY_ACCOUNT_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(WECHATPAY), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
    ...
    @Test(priority = 4, description = "Check whether the WECHATPAY payment order placed & failed through communication break")
    public void fourthOrder(){
        ...
        String tid = shopware.getNovalnetAdminPortal().getTID(ShopwareAPIs.getInstance().getCustomerEmail());
        String orderNumber = TID_Helper.getOrderNumber(tid);
        shopwareWeChatPay.getCallback().communicationBreakFailure(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
    ...
    public void fifthOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().cancelAtWeChatPayRedirectionPage();
