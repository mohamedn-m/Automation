...
public class Blik extends BaseTest {
    Shopware shopwareBlik = Shopware.builder()
    ...
    @Test(priority = 1, description = "Check whether the Blik payment order placed successfully and Callback partial refund events executed successfully")
    public void firstOrder() {
        ...
        verifyEquals(paymentName, getPaymentName(BLIK), PAYMENT_NAME_IN_SUCCESS_PAGE);
        //My account page
        ...
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(BLIK), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(BLIK), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
        ...
        var partialRefund = shopwareBlik.getCallback().transactionRefund(tid, String.valueOf(Integer.parseInt(totalAmount) / 2));
        ...
        verifyContains(shopware.getMyAccountPage().getComments(orderNumber), getCallbackResponse(partialRefund).replace("\u00A0", " ").trim(), TRANSACTION_COMMENT_IN_MY_ACCOUNT_PAGE);
    }
    @Test(priority = 2, description = "Check whether the Blik payment order placed successfully and Callback for full refund event executed successfully")
    public void secondOrder() {
        ...
        var partialRefund = shopwareBlik.getCallback().transactionRefund(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
    ...
    @Test(priority = 3, description = "Check whether the Blik payment order placed successfully through communication break")
    public void thirdOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().waitForBlikRedirectionPage();
        shopware.getNovalnetAdminPortal().openNovalnetAdminPortal();
        ...
        shopwareBlik.getCallback().communicationBreakSuccess(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
        verifyEquals(shopware.getMyAccountPage().getOrderStatus(orderNumber), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_MY_ACCOUNT_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber),getPaymentName(BLIK) , PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
    ...
    @Test(priority = 4, description = "Check whether the Blik payment order placed & failed by communication break")
    public void fourthOrder(){
        ...
        String tid = shopware.getNovalnetAdminPortal().getTID(ShopwareAPIs.getInstance().getCustomerEmail());
        String orderNumber = TID_Helper.getOrderNumber(tid);
        shopwareBlik.getCallback().communicationBreakFailure(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
    ...
    public void fifthOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().cancelOrderWithBlik();
