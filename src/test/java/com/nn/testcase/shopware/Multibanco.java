...
import static com.nn.language.NovalnetCommentsEN.getMultibancoComment;
...
public class Multibanco extends BaseTest {
    Shopware shopwareMul = Shopware.builder()
    ...
    @Test(priority = 1, description = "Check whether the Multibanco payment order placed successully and credit events executed successfully")
    public void firstOrder(){
        ...
        verifyEquals(initialComment, getMultibancoComment(tid), TRANSACTION_COMMENT_IN_SUCCESS_PAGE);
        verifyEquals(paymentName, getPaymentName(MULTIBANCO), PAYMENT_NAME_IN_SUCCESS_PAGE);
        //My account page
        ...
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(MULTIBANCO), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(MULTIBANCO), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
        ...
        var credit = shopwareMul.getCallback().multibancoCredit(tid,totalAmount);
    ...
    @Test(priority = 2, description = "Check whether the Multibanco payment order placed successully and partial credit events executed successfully")
    public void secondOrder(){
        ...
        var credit = shopwareMul.getCallback().multibancoCredit(tid,String.valueOf(Integer.parseInt(totalAmount)/2));
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyContains(shopware.getOrdersPage().getComments(), getCallbackResponse(credit), TRANSACTION_COMMENT_IN_ADMIN_ORDER_DETAIL_PAGE);
    ...
    @Test(priority = 3, description = "Check whether the Multibanco payment order placed successully and partial credit events executed successfully")
