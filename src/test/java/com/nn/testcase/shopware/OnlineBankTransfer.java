...
public class OnlineBankTransfer extends BaseTest {
    ...
    Shopware shopwareObt = Shopware.builder()
    ...
    @Test(priority = 1, description = "Check whether the ONLINE_BANK_TRANSFER payment order placed successfully and refund, chargeback, and credit events executed successfully")
    public void firstOrder() {
        ...
        verifyEquals(paymentName, getPaymentName(ONLINE_BANK_TRANSFER), PAYMENT_NAME_IN_SUCCESS_PAGE);
        //My account page
        ...
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(ONLINE_BANK_TRANSFER), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentNameInside(orderNumber), getPaymentName(ONLINE_BANK_TRANSFER), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_DETAIL_PAGE);
        ...
        var partialRefund = shopwareObt.getCallback().transactionRefund(tid, String.valueOf(Integer.parseInt(totalAmount) / 2));
        ...
        //chargeback
        var chargebackComment = shopwareObt.getCallback().chargeback(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        ...
        var onlineTransferCredit = shopwareObt.getCallback().onlineTransferCredit(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.CHARGEBACK.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
        ...
        //debtCollectionDE
        var debtCollectionCreditCard = shopwareObt.getCallback().debtCollectionDE(tid, totalAmount);
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
        verifyEquals(shopware.getOrdersPage().getOrderStatusNovalnet(), ShopwareOrderStatus.CHARGEBACK.get(), ORDER_STATUS_IN_ADMIN_ORDER_DETAIL_PAGE);
    ...
    @Test(priority = 2, description = "Check whether the ONLINE_BANK_TRANSFER payment order placed successfully and full refund events executed successfully")//,retryAnalyzer = RetryListener.class)
    public void secondOrder(){
        ...
        var partialRefund = shopwareObt.getCallback().transactionRefund(tid, String.valueOf(Integer.parseInt(totalAmount)));
        shopware.getOrdersPage().openOrdersPage().openOrderDetail(orderNumber);
    ...
    @Test(priority = 3, description = "Check whether the ONLINE_BANK_TRANSFER payment order placed successfully through communication break")
    public void thirdOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().waitForOnlineBankTransferRedirection();
        shopware.getNovalnetAdminPortal().openNovalnetAdminPortal();
        ...
        shopwareObt.getCallback().communicationBreakSuccess(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
        verifyEquals(shopware.getMyAccountPage().getOrderStatus(orderNumber), ShopwareOrderStatus.PAID.get(), ORDER_STATUS_IN_MY_ACCOUNT_PAGE);
        verifyEquals(shopware.getMyAccountPage().getPaymentName(orderNumber), getPaymentName(ONLINE_BANK_TRANSFER), PAYMENT_NAME_IN_MY_ACCOUNT_ORDER_LIST_PAGE);
    ...
    @Test(priority = 4, description = "Check whether the ONLINE_BANK_TRANSFER payment order placed & failed through communication break")
    public void fourthOrder(){
        ...
        String tid = shopware.getNovalnetAdminPortal().getTID(ShopwareAPIs.getInstance().getCustomerEmail());
        String orderNumber = TID_Helper.getOrderNumber(tid);
        shopwareObt.getCallback().communicationBreakFailure(tid, totalAmount);
        //My account page
        shopware.getMyAccountPage().load().openOrdersPage().expandOrder(orderNumber);
    ...
    public void fifthOrder(){
        ...
        shopware.getCheckoutPage()
                ...
                .exitIframe();
        var totalAmount = shopware.getCheckoutPage().getGrandTotalAmount();
        shopware.getCheckoutPage().clickSubmitOrderBtn().cancelAtOnlineBankTransferRedirection();
