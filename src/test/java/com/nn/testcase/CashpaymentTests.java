...
public class CashpaymentTests extends BaseTest {
    ...
    public void adminLogin(@Optional("oncash") String userName, @Optional("wordpress") String password) {
        wooCommerce.getAdminPage().openAdminPage();
        wooCommerce.getAdminPage().adminLogin(userName, password);
    }

    @Test(priority = 1, description = "Check whether the oncash payment order placed successfully and refund via admin executed successfully",retryAnalyzer = RetryListener.class)
    public void firstOrder(){
        ...
        TID_Helper.verifyTIDInformation(wooCommerce.getTxnInfo().get("TID").toString(), wooCommerce.getTxnInfo().get("TotalAmount").toString(), TID_STATUS_PENDING, CASHPAYMENT,14);
        TID_Helper.verifyNearestStoresExists(wooCommerce.getTxnInfo().get("TID").toString());
        statusCommentsVerification(PROCESSING_ORDER_STATUS);
        partialRefundAdmin();
        fullRefundAdmin();
    }

    @Test(priority = 2, description = "Check whether the oncash payment order placed successfully and due date update, credit, refund via callback events executed successfully",retryAnalyzer = RetryListener.class)
    public void secondOrder(){
        ...
        wooCommerce.getSuccessPage().verifyNovalnetCommentsDisplayed();
        wooCommerce.getTxnInfo().putAll(wooCommerce.getSuccessPage().getSuccessPageTransactionDetails());
        TID_Helper.verifyTIDInformation(wooCommerce.getTxnInfo().get("TID").toString(), wooCommerce.getTxnInfo().get("TotalAmount").toString(), TID_STATUS_PENDING, CASHPAYMENT,10);
        TID_Helper.verifyNearestStoresExists(wooCommerce.getTxnInfo().get("TID").toString());
    ...
    @Test(priority = 3, description = "Check whether the on cash payment order placed successfully and amount update, credit, refund via callback events executed successfully",retryAnalyzer = RetryListener.class)
    ...
    public void guestOrder(){
        ...
        wooCommerce.getAdminPage().adminLogin("oncash", "wordpress");
    ...
    @Test(priority = 5,description = "Check whether Cash payment logo displayed ")
    public void verifyCashPaymentLogoDisplayed(){
        wooCommerce.getAdminPage().ifLoggedOutLogin("oncash","wordpress");
        setPaymentConfiguration(true,false,"");
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isOnCashPaymentDisplayed();
        wooCommerce.getCheckoutPage().isLogoDisplayed("Barzahlen/viacash");
    }
    @Test(priority = 6, description = "Verify that payment is hidden on the checkout page when payment is disabled in shop backend")
    public void deactivatePaymentVerifyCheckout(){
        wooCommerce.getAdminPage().ifLoggedOutLogin("oncash","wordpress");
        wooCommerce.getSettingsPage().paymentPageLoad();
        wooCommerce.getSettingsPage().activatePayment(CASHPAYMENT,false);
        navigateCheckout(PRODUCT_1);
        wooCommerce.getCheckoutPage().isPaymentDisplayed(CASHPAYMENT,false);
    }
    ...
    public void invoiceCredit(String amount,String status) {
        wooCommerce.getCallback().cashpaymentCredit(wooCommerce.getTxnInfo().get("TID").toString(), amount);
        wooCommerce.getOrdersPage().load();
    ...
    public void amountUpdate() {
        ...
        wooCommerce.getOrdersPage().getOrderNoteComment(UPDATE_COMMENT_);
        wooCommerce.getMyAccountPage().loadOrders();
        wooCommerce.getMyAccountPage().clickOrder(wooCommerce.getTxnInfo().get("OrderNumber").toString());
